...
"""
KimiK2ToolParser with large-payload and streaming correctness fixes.

This ports the core bug fixes from upstream PR #37384 while keeping the
change scoped to the Kimi parser itself.
"""
...
class _ParsedToolCall(TypedDict):
    id: str
    name: str
    arguments: str
...
class KimiK2ToolParser(ToolParser):
    ...
    def _parse_tool_call_portion(self, portion: str) -> _ParsedToolCall | None:
        match = self.stream_tool_call_portion_regex.match(portion)
        if match:
            call_id = match.group("tool_call_id").strip()
            return {
                "id": call_id,
        ...
        match = self.stream_tool_call_name_regex.match(portion)
        if match:
            call_id = match.group("tool_call_id").strip()
            return {
                "id": call_id,
    ...
    def _diff_and_emit_args(self, cur_args: str) -> DeltaMessage | None:
        ...
        if already and not cur_args.startswith(already):
            logger.error(
                "Non-monotonic tool argument change detected. "
                "Previous=%r Current=%r. Suppressing delta to avoid "
        ...
        return DeltaMessage(
            tool_calls=[
                DeltaToolCall(
                    ...
                    type="function",
                    id=parsed["id"],
                    function=DeltaFunctionCall(name=parsed["name"]),
                )
            ]
        )
    ...
    def extract_tool_calls(
        ...
        section_begin = next(
            (variant for variant in self.tool_calls_start_token_variants
                ...
                tool_calls.append(
                    ToolCall(
                        ...
                        function=FunctionCall(
                            name=self._call_id_to_name(function_id),
    ...
    def extract_tool_calls_streaming(
        ...
        has_section_token = any(
            token_id in current_token_ids
            for token_id in self.tool_calls_start_token_ids
        )
        ...
        try:
            ...
            if cur_start_count > cur_end_count and cur_start_count > prev_start_count:
                ...
                self.prev_tool_call_arr.append({"id": "", "name": "", "arguments": ""})
